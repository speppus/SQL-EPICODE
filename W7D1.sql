...
select pc.EnglishProductCategoryName as categoria , sum(r.OrderQuantity) as quantita_totale, sum(r.SalesAmount) as fatturato_totale
from factresellersales r
inner join dimproduct p
on r.ProductKey = p.ProductKey
...
select sum(r.SalesAmount) fatturato, r.OrderDate,g.City
from factresellersales r
inner join dimgeography g
on r.SalesTerritoryKey = g.SalesTerritoryKey
where r.OrderDate >= '2020-01-01'
group by g.City
having sum(r.SalesAmount) >= "60000";
